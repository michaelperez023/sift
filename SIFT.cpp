...
void sift::DoGExtrema()
{
	...
	for (int oct = 0; oct < num_octaves; oct++)
	{
		for (int sc = 0; sc < num_scales; sc++)
		{
			...
			for (int x = 0; x < extrema.rows; x++)
			{
				for (int y = 0; y < extrema.cols; y++)
				{
					if (extrema.at<Vec3b>(y, x) == Vec3b(0, 0, 255))
					{
						if (oct == 0)
							circle(step1, Point(x / 2, y / 2), 10, (255, 0, 0), 1);
						else if (oct == 1)
							circle(step1, Point(x, y), 10, (255, 0, 0), 1);
						else if (oct == 2)
							circle(step1, Point(x * 2, y * 2), 10, (255, 0, 0), 1);
						else if (oct == 3)
							circle(step1, Point(x * 4, y * 4), 10, (255, 0, 0), 1);
					}
				}
			}
		}
	...
	imshow("Step 1 Result", step1);
}

// function to filter out extrema with low contrast or on edges
void sift::FilterDoGExtrema()
{
	...
	for (int oct = 0; oct < num_octaves; oct++)
	{
		for (int sc = 0; sc < num_scales; sc++)
		{
			...
			findNonZero(DoG_Keypts[oct][sc], locs);       // locations of key points
			...
			for (int k = 0; k < num_keypts; k++)
			{
				...
				if(oct == 0)
					circle(step2, Point(x/2, y/2), 10, (255, 0, 0), 1);
				else if (oct == 1)
					circle(step2, Point(x, y), 10, (255, 0, 0), 1);
				else if (oct == 2)
					circle(step2, Point(x*2, y*2), 10, (255, 0, 0), 1);
				else if (oct == 3)
					circle(step2, Point(x*4, y*4), 10, (255, 0, 0), 1);
			}
			cout << "Octave: " << oct + 1 << " Scale: " << sc + 1 << " Keypoints: " << num_keypts << " Rejected (" << reject_contrast_count << " contrast, " << reject_edge_count << " edge): " << reject_contrast_count + reject_edge_count << endl;
		}
	}
	imshow("Step 2 Result", step2);

}

// function to assign gradient direction and magnitude of the gaussian pyramid images in Scale Space
		...
		for (int sc = 0; sc < num_scales; sc++)
		{
			...
			for (int x = 1; x < current.rows - 1; x++)
			{
				for (int y = 1; y < current.cols - 1; y++)
				{
					...
					cout << "Location[" << x << "][" << y << "]: Magnitude = " << Magnitude[oct][sc].at<uchar>(x, y) << "Orientation = " << Orientation[oct][sc].at<uchar>(x, y) << endl;
